...
for (const [type, sold] of cases) {
  const pt = await ev(`(() => {
    const bs = [...document.querySelectorAll('[data-bays] [data-bay]')].filter(b => b.dataset.type === '${type}' && b.dataset.sold === '${sold}')
...
/* and the selection */
const pick = await ev(`(() => { const bs=[...document.querySelectorAll('[data-bays] [data-bay]')].filter(b=>b.dataset.sold!=='true')
