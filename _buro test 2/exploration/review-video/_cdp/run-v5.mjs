...
await ev(`(() => {
  ...
  b.innerHTML = '<b style="color:#6fe08a">V5 — LIVE guided sales experience</b>' +
...
const rec = recorder(cdp, 'exploration/review-video/_cdp/frames-v5')
...
await cap('HOVER 03 — the buildings OWN PARAPET lights; the others step back')
await sleep(3800)

await ev(`window.__v5.selectBuilding(window.__v5.compound.byNum.get('03'))`)
await cap('SELECT 03 — camera moves, the parapet turns red, the bays appear')
await sleep(4400)
await cap('MINIATURE BAYS — the wide ones are PREMIUM: 30ft against 23ft, as built')
await sleep(3200)
...
await sleep(3200)
await cap('CLICK IT')
/* re-read the bay immediately before pressing: the dock settles into place after the
   camera move and a coordinate taken a second earlier can miss by a whole unit */
const bay2 = await ev("(() => { const b=[...document.querySelectorAll('.bay')].filter(x=>x.dataset.sold!=='true')[10]; const r=b.getBoundingClientRect(); return [Math.round(r.left+r.width/2), Math.round(r.top+r.height/2)]; })()")
await clickAt(cdp, bay2)
await sleep(4200)
await cap('SUITE SELECTED — type, area, price, and ENTER arms')
await sleep(3400)

const go = await rectOf('.enter')
if (go) { cur = await moveTo(cdp, cur, go, 26, 20); await cap('ENTER — armed only now, and it fills under the pointer'); await sleep(3400) }

const ovr = await rectOf('#overview')
if (ovr) { cur = await moveTo(cdp, cur, ovr, 26, 20); await cap('ONE CLICK — OVERVIEW returns the compound'); await clickAt(cdp, ovr); await sleep(3800) }
await cap('AND BACK IN — building 03, suite 13'); await ev("window.__v5.selectBuilding(window.__v5.compound.byNum.get('03'))"); await sleep(2600)
await ev('window.__v5.selectSuite(window.__v5.S.building.suites.filter(s=>!s.sold)[10])'); await sleep(3200)
await cap('OPTIONAL — SITE PLAN')
